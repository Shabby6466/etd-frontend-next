...
import {
  parseSheetNumbers,
  validateSheetNumbers,
} from "@/lib/utils/sheet-parser";
...
export function SheetManagementTable() {
  ...
  const applyFilters = () => {
    setFilters((prev) => ({
      ...prev,
      operator_id:
        filterInputs.operator_id && filterInputs.operator_id !== "all"
          ? parseInt(filterInputs.operator_id)
          : undefined,
      location_id:
        filterInputs.location_id && filterInputs.location_id !== "all"
          ? parseInt(filterInputs.location_id)
          : undefined,
      status:
        filterInputs.status && filterInputs.status !== "all"
          ? (filterInputs.status as "EMPTY" | "USED" | "QC_PASS" | "QC_FAIL")
          : undefined,
      number:
        filterInputs.number && filterInputs.number.trim() !== ""
          ? filterInputs.number.trim()
          : undefined,
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="text-xs text-gray-500 mb-2">
                Enter individual numbers (comma or newline separated) or use
                ranges like "100-104" to assign multiple sheets at once
              </div>
              <textarea
                id="sheetNumbers"
                value={sheetNumbers}
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            <div className="space-y-2 rounded-3xl">
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setFilterInputs((prev) => ({ ...prev, operator_id: value }))
                }
              >
                <SelectTrigger>
                      ...
                      >
                        ...
                        <td className="p-3">
                          <div className="flex justify-center">
                            <Badge
                              variant={
                                sheet.status === "QC_PASS"
                                  ? "default"
                                  : sheet.status === "QC_FAIL"
                                  ? "destructive"
                                  : "secondary"
                              }
                              className={
          ...
          {sheets && sheets.length > 0 && pagination.totalPages > 1 && (
            <div className="flex items-center justify-between mt-6">
              <div className="text-sm text-gray-600">
                ...
                of {pagination.totalItems} sheets
              </div>
