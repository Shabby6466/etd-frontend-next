...
This application provides a desktop interface for the ETD system with native
biometric device integration using Python libraries.
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('etd_app.log'),
...
class BiometricDevice:
    ...
    def __init__(self):
        ...
        self.device_name = "HU20"
        self.serial_number = "H58220311290"
    ...
    def test_connection(self) -> Dict[str, Any]:
        ...
        try:
            url = f"{self.base_url}/SGIFPCapture"
            params = {
                'Timeout': '3000',
            ...
            logger.info(f"Biometric device response: {data}")
            return {
                'success': True,
                'data': data,
                'error': None
            ...
            logger.error(f"Biometric device connection failed: {e}")
            return {
                'success': False,
                'data': None,
                'error': str(e)
    ...
    def capture_fingerprint(self, timeout: int = 25000) -> Dict[str, Any]:
...
class DataInputScreen:
    """Data input screen for citizen information"""
    ...
    def __init__(self, parent, user, on_logout, on_back):
        ...
        self.frame = ttk.Frame(parent)
        self.biometric_device = BiometricDevice()
        self.setup_ui()
    
    def setup_ui(self):
        """Setup the data input UI"""
    ...
    def setup_form_fields(self, parent):
        ...
        # Title
        title_label = ttk.Label(parent, text="Citizen Information", font=('Arial', 18, 'bold'))
        ...
        self.setup_biometric_section(parent)
    ...
    def setup_biometric_section(self, parent):
        """Setup biometric capture section"""
        # Biometric section
        biometric_frame = ttk.LabelFrame(parent, text="Biometric Capture", padding=10)
        biometric_frame.pack(fill='x', pady=20)
        
        # Device status
        self.device_status_label = ttk.Label(biometric_frame, text="Checking device...")
        self.device_status_label.pack(pady=(0, 10))
        
        # Test connection button
        test_btn = ttk.Button(
            biometric_frame, 
            text="Test Device Connection", 
            command=self.test_biometric_device
        )
        ...
        # Test device connection on startup
        self.test_biometric_device()
    
    def test_biometric_device(self):
        """Test biometric device connection"""
        def test_in_thread():
            result = self.biometric_device.test_connection()
            self.parent.after(0, lambda: self.handle_device_test_result(result))
        
        threading.Thread(target=test_in_thread, daemon=True).start()
    
    def handle_device_test_result(self, result):
        """Handle device test result"""
        if result['success'] and result['data'].get('ErrorCode') == 0:
            device_info = result['data']
            self.device_status_label.config(
                text=f"✅ Device Connected: {device_info.get('Model', 'Unknown')} (Serial: {device_info.get('SerialNumber', 'Unknown')})",
                foreground='green'
            )
    ...
    def capture_fingerprint(self):
        ...
        def capture_in_thread():
            result = self.biometric_device.capture_fingerprint()
            self.parent.after(0, lambda: self.handle_capture_result(result))
    ...
    def handle_capture_result(self, result):
        ...
        if result['success'] and result['data'].get('ErrorCode') == 0:
            ...
            error_msg = result.get('error', 'Unknown error')
            self.biometric_data_label.config(
                text=f"❌ Capture failed: {error_msg}",
                foreground='red'
            )
    ...
    def handle_submit(self):
        ...
        # Log the submission
        logger.info(f"Form submitted: {json.dumps(form_data, indent=2)}")
...
class ETDApplication:
    """Main ETD application class"""
    
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("ETD Desktop Application")
    ...
    def show_data_input_screen(self):
        ...
        self.clear_screen()
        self.current_screen = DataInputScreen(
            self.root, 
            self.user, 
            self.on_logout,
            self.on_back
