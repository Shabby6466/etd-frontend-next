#!/usr/bin/env python3
"""
Enhanced Biometric Device Integration with Comprehensive Logging
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if any(keyword in result.stdout.lower() for keyword in ['secugen', 'hu20', 'fingerprint']):
                    self.logger.info("✅ SecuGen device detected in USB devices")
                else:
                    self.logger.warning("❌ No SecuGen device found in USB devices")
            else:
        ...
        try:
            ...
            for key_path in key_paths:
                try:
                    ...
                    self.logger.info(f"✅ Registry key found: {key_path}")
                    winreg.CloseKey(key)
        ...
        for attempt in range(self.max_retries):
            ...
            try:
                ...
                if result['success']:
                    ...
                    self.logger.warning(f"❌ Connection failed on attempt {attempt + 1}: {result.get('error')}")
    ...
    def capture_fingerprint_with_retry(self, timeout: Optional[int] = None) -> Dict[str, Any]:
        ...
        for attempt in range(self.max_retries):
            ...
            try:
                ...
                if result['success']:
                    self.logger.info(f"✅ Fingerprint captured successfully on attempt {attempt + 1}")
                    return result
                else:
                    self.logger.warning(f"❌ Capture failed on attempt {attempt + 1}: {result.get('error')}")
            ...
            if response.status_code == 200:
                try:
                    ...
                    if error_code == 0:
                        ...
                        self.logger.info(f"✅ Fingerprint captured - Quality: {quality}, NFIQ: {nfiq}, Template length: {len(template)}")
                        ...
                        error_msg = self._get_error_message(error_code)
                        self.logger.warning(f"❌ Capture error {error_code}: {error_msg}")
                        return {
                            'success': False,
                            'data': data,
